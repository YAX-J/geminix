...
CREATE TABLE issue (
  ...
  solution    TEXT         NULL COMMENT '解决方案（为空表示未解决）',
...
INSERT INTO issue (title, description, solution, tag, status, report_date) VALUES
...
('大列表渲染卡顿掉帧', '5 万行报表明细渲染导致滚动掉帧', '虚拟滚动 + 分页加载，仅渲染可视区域，首屏 2.3s → 0.4s', '前端', 'done', '2026-08-07'),
('网关 Token 校验重复查 Redis', '每个请求重复解析 Token，多次查 Redis，接口延迟增加约 40ms', '请求链路内 Token 只解析一次 + 黑名单机制，耗时降低 60%', '后端', 'done', '2026-08-06'),
('跨域请求被浏览器拦截', '本地开发环境跨域请求被拦截，接口无法联调', '网关 CORS 白名单 + Vite/Nginx 代理转发', '前端', 'done', '2026-08-05'),
('Docker 容器内存溢出', '测试环境容器运行一段时间后被 OOM Kill', 'JVM 堆参数与容器限额对齐 + 健康检查自动重启', '运维', 'done', '2026-08-04'),
('定时任务偶发重复执行', '分布式环境下定时任务被多个节点同时触发，导致数据重复写入', '', '后端', 'open', NULL),
('图片上传偶现 413 错误', '上传大图时网关返回 413，怀疑 Nginx client_max_body_size 限制', '', '运维', 'open', NULL);
